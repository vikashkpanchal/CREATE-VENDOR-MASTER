...
Deliberately dependency-free (no matplotlib): the dashboard only needs
ranked magnitude comparisons, which are horizontal bars.

Design follows the house data-viz rules: one hue for a single-measure
ranking (colour encodes nothing here, so it must not vary by rank),
direct value labels instead of a value axis, recessive gridlines, thin
marks with rounded data-ends, and a hover tooltip on every bar.
"""
